...
function getPersonaSignature() {
    if (selectedAdminPersona === 'Micha') return 'Michaaaaa';
    if (selectedAdminPersona === 'Lukas') return 'Lukaaaas';
    return '';
}

function displayGlobalMessage(payload) {
    if (!payload || !payload.text) return;
    const signature = getPersonaSignature();
